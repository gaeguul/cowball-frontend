    ...
    return (
        <>
            ...
            <tr>
                ...
                <td className={rsvAmount ? 'td-rsv' : 'td-empty'}><span>{rsvAmount}</span></td>
                <td className={outAmount ? 'td-out' : 'td-empty'}><span>{outAmount}</span></td>
                {editableCategory.some(c => c === category.categoryName)
                    ? <td className={inAmount ? 'td-editable td-in' : 'td-editable td-empty'} onClick={setInAmountEditDialog}>
                        <span>
                            {inAmount}
                    ...
                    : <td className={inAmount ? 'td-in': 'td-empty'}><span>{inAmount}</span></td>    
                }
            </tr>
        </>
    );
        ...
        <table className='ingredient-list-table'>
            ...
            <tbody>
                {ingredientList.map((ingredient, index) => {
                    const category = ingredientCategories.filter(value => value.categoryId === ingredient.categoryId)[0];
                    const schedule = todayIngredientSchedule.filter(value => value.ingredientId === ingredient.ingredientId)
                    ...
                    return (
                        <IngredientRow
                            ...
                            requireRefresh={() => {
                                API.getTodayIngredientSchedule().then(setTodayIngredientSchedule);
                                API.getIngredient(ingredient.ingredientId).then(data => data.stock).then(stock => {
