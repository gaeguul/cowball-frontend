    ...
    <table className='ingredient-schedule-table'>
      <thead>
        <tr>
          ...
          {schedule &&
            schedule.map((col, index) => (
              <th
                colSpan={4}
                key={index}
                className={index === todayIndex ? 'th-today' : ''}
              >
        ...
        <tr>
          {schedule.map((s, index) => {
            ...
            return (
              <>
                <th className={className} key={index * 10}>
                  예약
                </th>
                <th className={className} key={index * 10 + 1}>
                  입고
                </th>
                <th className={className} key={index * 10 + 2}>
                  출고
                </th>
                <th className={className} key={index * 10 + 3}>
                  주문
                </th>
              </>
            );
            ...
            return (
              <tr key={index}>
                ...
                <td
                  className='td-amount'
                  onClick={() => {
                    setNumberPickOpen(true);
                  }}
                >
                  {ing.stock}
                </td>
                {schedule.map((s, index) => {
                  const itemQuery = s.items.filter(
                    (value, ix) => value.ingredientId === id,
                  );
                  ...
                  return nums.map((num, i) => (
                    <td key={i} className={num ? classNames[i] : 'td-empty'}>
                      {num}
                    </td>
                  ));
                })}
